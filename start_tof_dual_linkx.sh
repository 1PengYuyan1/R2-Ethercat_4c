...
START_UP_RANGE_SUBSCRIBER="${START_UP_RANGE_SUBSCRIBER:-true}"
...
echo "[TOF] ifname=${IFNAME} can_baud=${CAN_BAUD} high_front=${SLAVE_HIGH_FRONT} high_up_down=${SLAVE_HIGH_UP_DOWN} low=${SLAVE_LOW}"
...
CMD="
  ...
  '${WS_DIR}/install/tof/lib/tof/can_link_test' '${IFNAME}' '${CAN_BAUD}' '${SLAVE_HIGH_FRONT}' '${SLAVE_HIGH_UP_DOWN}' '${SLAVE_LOW}'
"

if [[ "${START_UP_RANGE_SUBSCRIBER}" == "true" ]]; then
  CMD="
    source /opt/ros/humble/setup.bash &&
    source '${WS_DIR}/install/setup.bash' &&
    export FASTRTPS_DEFAULT_PROFILES_FILE='${FASTRTPS_DEFAULT_PROFILES_FILE}' &&
    ...
    SUB_PID=\$! &&
    trap 'kill \${SUB_PID} >/dev/null 2>&1 || true' EXIT INT TERM &&
    '${WS_DIR}/install/tof/lib/tof/can_link_test' '${IFNAME}' '${CAN_BAUD}' '${SLAVE_HIGH_FRONT}' '${SLAVE_HIGH_UP_DOWN}' '${SLAVE_LOW}'
  "
fi
