 ...
 *    - ODrive 速度 CSV / BRT 编码器仪表盘 / OPS 探针 1Hz 输出
 */
...
linkx_t       linkx_dev;    ///< LinkX 4 通道 CAN 适配器（1Mbps 经典 CAN）
...
static void Print_Live_Dashboard()
{
    ...
    {
        auto &gl = robot.Gantry.Motor_Lift_Left;
        auto &gr = robot.Gantry.Motor_Lift_Right;
        auto &am = robot.Arm.Arm;
        n += std::snprintf(buf + n, kBufSize - n,
                           "\n[GANTRY]\n  L tx=0x%02X rad=%+7.3f omega=%+6.3f torque=%+5.2f status=%d\n"
                           "  R tx=0x%02X rad=%+7.3f omega=%+6.3f torque=%+5.2f status=%d\n"
                           "  gantry_ctrl=%d\n"
                           "\n[ARM]\n  A tx=0x%02X rad=%+7.3f omega=%+6.3f torque=%+5.2f status=%d arm_ctrl=%d\n",
                           (unsigned)gl.DM_CAN_Tx_ID, gl.Get_Now_Radian(), gl.Get_Now_Omega(),
                           gl.Get_Now_Torque(), (int)gl.Get_Status(),
                           (unsigned)gr.DM_CAN_Tx_ID, gr.Get_Now_Radian(), gr.Get_Now_Omega(),
                           gr.Get_Now_Torque(), (int)gr.Get_Status(),
                           (int)robot.Gantry.Gantry_Control_Type,
                           (unsigned)am.DM_CAN_Tx_ID, am.Get_Now_Radian(), am.Get_Now_Omega(),
                           am.Get_Now_Torque(), (int)am.Get_Status(),
                           (int)robot.Arm.Arm_Control_Type);
    }
...
/**
 * @brief EtherCAT 主站 + 单 LinkX 4 通道 1Mbps 经典 CAN 初始化。
...
static void Init_Robot_Logic()
{
    ...
    std::cout << "[TASK] R2 robot logic initialized." << std::endl;
}
 ...
 *
 *  对象（共 11 个执行器）：
 *    - 4× DM6225 舵向（ch0, ID 0x01-0x04）
 *    - 4× DM3519 轮向（ch1, ID 0x01-0x04）
 *    - 2× DM 龙门（ch2, ID 0x01/0x02）
 *    - 1× DM 机械臂（ch2, ID 0x03）
 *
...
static void Disable_All_Devices()
{
    ...
    for (int cycle = 0; cycle < kDisableSendCycles; ++cycle)
    {
        ...
        robot.Gantry.Motor_Lift_Right.CAN_Send_Exit();
                               ...
                               std::chrono::steady_clock::time_point target)
{
    ...
    {
        std::fprintf(stderr, "[TASK][DRIFT] tick=%u drift=%lldus\n",
...
void Robot_Control_Loop(const char *ifname)
{
    ...
    std::cout << "[TASK] Robot_Control_Loop exited cleanly." << std::endl;
}
